...
def save_model_checkpoint(
    ...
    if hasattr(solver, "model") and solver.model is not None:
        ...
        logger.warning(
            f"❌ Solver has no model (has model: {hasattr(solver, 'model')}, is None: {solver.model is None if hasattr(solver, 'model') else 'N/A'})"
        )
...
def load_model_checkpoint(
    solver: SolverAgent, checkpoint_path: Path, proposer: ProposerAgent
) -> float:
    ...
    if "model_state_dict" in checkpoint:
        try:
            if (
                hasattr(solver, "model")
                and solver.model is not None
                and hasattr(solver.model, "model")
            ):
                ...
                logger.info(
                    f"✅ Loaded model weights from episode {checkpoint['episode']}"
                )
        except Exception as e:
...
def train_selfplay_model(
    ...
    for episode in range(num_episodes):
        ...
        if episode > 0 and episode % 10 == 0:
            ...
            logger.info(
                f"📈 Curriculum: {old_curriculum:.1f} → {proposer.curriculum_level:.1f} (episode {episode})"
            )
        ...
        if episode % validation_frequency == 0:
            ...
            logger.info(
                f"📊 Episode {episode}: train_mae={train_mae:.4f}, val_mae={val_mae:.4f}, curriculum={proposer.curriculum_level:.1f}"
            )
            ...
            if val_mae < best_val_mae:
                ...
                save_model_checkpoint(
                    solver, checkpoint_path, episode, val_mae, train_mae, proposer
                )
                logger.info(
                    f"💾 Best checkpoint saved (episode {episode}, val_mae={val_mae:.4f})"
                )
            else:
                patience_counter += 1
            ...
            if patience_counter >= early_stopping_patience:
                logger.info(
                    f"🛑 Early stopping at episode {episode} (no improvement for {early_stopping_patience * validation_frequency} episodes)"
                )
                break
    ...
    last_val_episode = max(
        [m["episode"] for m in episode_metrics if m.get("val_mae") is not None],
        default=-validation_frequency,
    )
    if episode - last_val_episode >= validation_frequency:
        ...
        if final_val_mae < best_val_mae:
            logger.info("💡 Final model is best! Saving...")
            best_val_mae = final_val_mae
            best_episode = episode
            checkpoint_path = output_dir / f"best_checkpoint_run{run_id}.pkl"
            save_model_checkpoint(
                solver, checkpoint_path, episode, final_val_mae, train_mae, proposer
            )
    ...
    if checkpoint_path.exists():
        ...
        logger.info(
            f"✅ Loaded best model from episode {best_episode} (val_mae={loaded_best_mae:.4f})"
        )
    ...
    if abs(mae - best_val_mae) / best_val_mae > 0.10:
        logger.warning("⚠️ Final MAE is >10% different from best MAE")
    else:
        logger.info("✅ Checkpoint loading verified (within 10%)")
    ...
    if abs(proposer.curriculum_level - expected_curriculum) > 0.05:
        logger.warning(
            f"⚠️ Curriculum {proposer.curriculum_level:.1f} doesn't match expected {expected_curriculum:.1f}"
        )
    elif proposer.curriculum_level >= 0.9:
        logger.info(
            f"✅ Curriculum fully progressed to {proposer.curriculum_level:.1f}"
        )
    else:
        logger.info(
            f"✅ Curriculum correctly at {proposer.curriculum_level:.1f} for {len(episode_metrics)} episodes"
        )

    if not any(m["train_mae"] > 0 for m in episode_metrics):
        logger.warning("⚠️ All training MAEs are zero")
    else:
        logger.info("✅ Training metrics logged correctly")
...
def create_visualizations(
    ...
    for method, method_stats in results["statistical_tests"].items():
        if "mean" in method_stats:
            ...
            lower_err = max(0, method_stats["mean"] - method_stats["ci_lower"])
            upper_err = max(0, method_stats["ci_upper"] - method_stats["mean"])
