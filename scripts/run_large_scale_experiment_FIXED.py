...
def train_selfplay_model(
    ...
    for episode in range(num_episodes):
        ...
        if episode % 10 == 0:
            ...
            logger.info(f"📊 Episode {episode}: train_mae={train_mae:.4f}, val_mae={val_mae:.4f}, curriculum={proposer.curriculum_level:.2f}")
            ...
            if val_mae < best_val_mae:
                ...
                logger.info(f"💾 Saved best checkpoint at episode {episode} (val_mae={val_mae:.4f})")
            else:
                patience_counter += 1
                
            # FIX #4: Early stopping
            if patience_counter >= early_stopping_patience:
                logger.info(f"🛑 Early stopping at episode {episode} (no improvement for {early_stopping_patience} episodes)")
                break
        ...
        if episode > 0 and episode % 10 == 0:
            ...
            logger.info(f"📈 Curriculum increased: {old_curriculum:.2f} → {proposer.curriculum_level:.2f}")
    ...
    if checkpoint_path.exists():
        ...
        logger.info(f"✅ Loaded best model from episode {checkpoint['episode']} (val_mae={checkpoint['val_mae']:.4f})")
    ...
    if abs(mae - best_val_mae) / best_val_mae > 0.15:
        logger.warning(f"⚠️ Final MAE is >15% different from best MAE - checkpoint may not have loaded correctly")
    if proposer.curriculum_level == 0.0:
        logger.warning(f"⚠️ Curriculum did not progress from 0.0")
    if not any(m['train_mae'] > 0 for m in episode_metrics):
        logger.warning(f"⚠️ All training MAEs are zero - logging may be broken")
        ...
        for run in results["detailed_results"]["selfplay"]:
            if "episode_history" in run:
                ...
                if episodes:
                    ...
                    ax1.plot(episodes, mae_values, 'b-', alpha=0.6, label='Validation MAE')
                    ax1.set_xlabel('Episode')
                    ax1.set_ylabel('Validation MAE (kWh)', color='b')
                    ax1.tick_params(axis='y', labelcolor='b')
                    
                    # Plot difficulty on right axis
                    ax2.step(episodes, difficulties, 'r-', alpha=0.8, where='post', label='Curriculum Level')
